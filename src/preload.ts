...
const start = async (): Promise<void> => {
  console.log('[AIDA.Connect Desktop] preload.ts start');
  const serverUrl = await invoke('server-view/get-url');

  if (!serverUrl) {
    console.log('[AIDA.Connect Desktop] preload.ts serverUrl is not defined');
    return;
  }
  ...
  console.log('[AIDA.Connect Desktop] waiting for RocketChatDesktop.onReady');
  RocketChatDesktop.onReady(() => {
    console.log('[AIDA.Connect Desktop] RocketChatDesktop.onReady fired');
    listen(
...
console.log('[AIDA.Connect Desktop] waiting for window load');
